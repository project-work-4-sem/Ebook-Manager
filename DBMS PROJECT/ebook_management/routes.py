...
from ebook_management.models import books, reader, reader_books, reader_books_review,Author
...
@login_required
def DeleteBooks():
    q=db.engine.execute("SELECT book_id,title,category,path FROM books NATURAL JOIN reader_books WHERE reader_id=(?)",(current_user.reader_id))
    books=q.fetchall()
...
def SearchBooks():
    ...
    books=q.fetchall()
    q3=db.engine.execute("SELECT author_name from author NATURAL JOIN books where books.author_id =author.author_id")
    author_name=q3.fetchone()
    return render_template('search.html', books=books,authors=author_name)

@app.route("/books/<int:book_id>",methods=['GET', 'POST'])
def Books(book_id):
    q3=db.engine.execute("SELECT author_name from author NATURAL JOIN books where book_id =(?)",book_id)
    author_name=q3.fetchone()
    q2=db.engine.execute("SELECT * FROM reader_books_review WHERE book_id=(?)",book_id)
    reviews=q2.fetchall()
    form=AddReviewForm()
    ...
    return render_template('Book.html',book=book,reviews=reviews,authors=authors,present=present,form=form,form2=form2,owned=owned,author_name=author_name)
...
@app.route("/AuthorBooks/<int:author_id>/<author_Name>")
@login_required
def AuthorBooks(author_id,author_Name):
    q=db.engine.execute("SELECT book_id,title,link,isbn,path,category FROM books WHERE author_id=(?)",(author_id))
    books=q.fetchall()
    
    return render_template('AuthorBooks.html', books=books,author_Name=author_Name)

@app.route('/SearchBooksAlphabet/<letter>',methods=['GET','POST'])
@login_required
def SearchBooksAlphabet(letter):
    q=db.engine.execute("SELECT * FROM books WHERE title LIKE (?) ",('{}%'.format(letter)))
    books=q.fetchall()
    return render_template('search.html', books=books)

@app.route('/SearchBooksYear/<int:start>',methods=['GET','POST'])
@login_required
def SearchBooksYear(start):
    if start>2001:
        q=db.engine.execute("SELECT * FROM books WHERE year>=(?) AND year<=2017",(start))
        books=q.fetchall()
        return render_template('search.html', books=books)
    else :
        start=request.form['Start']
        end=request.form['End']
        q=db.engine.execute("SELECT * FROM books WHERE year>=(?) AND year<=(?)",(start,end))
        books=q.fetchall()
        return render_template('search.html', books=books)
